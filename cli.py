...
from src.utils.data_helpers import load_papers_from_csv, save_results_to_csv, extract_and_update_csv
...
Examples:
  ...
  python cli.py papers.csv --output results.csv --limit 50
    ...
    parser.add_argument('--extract-abstracts', action='store_true', help='Extract missing abstracts from PDF files')
    parser.add_argument('--extract-only', action='store_true', help='Only extract abstracts from PDFs and save updated CSV (no pipeline processing)')
    ...
    if args.extract_only:
        try:
            extract_and_update_csv(args.input_csv, limit=args.limit)
            return 0
    ...
    try:
        papers = load_papers_from_csv(
            args.input_csv,
            limit=args.limit,
            skip_no_abstracts=args.skip_no_abstracts,
            extract_from_pdfs=args.extract_abstracts
        )
